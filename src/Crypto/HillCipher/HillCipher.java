...
// return "hello";
        ...
        for (int i = 0; i < row; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                if(k>=key.length()){
                    ...
                    keyMatrix[i][j] = alphabet.indexOf(key.charAt(k));
                    k++;
                }
            }
        }
    ...
    static void encrypt(int cipherMatrix[][], int keyMatrix[][], int messageVector[][],int row, int Prow)
    {
        int x, i, j;
        for (i = 0; i < row; i++)
        {
